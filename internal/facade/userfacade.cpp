...
/*
#include <lib/crypt_blowfish/ow-crypt.h>

#include <lib/crypt_blowfish/bcrypt.h>
#include <assert.h>
#include <stdio.h>
#include <string.h>
#include <time.h>
...
QJsonObject UserFacade::Login(UserLoginDTO userLoginDTO)
{
    User *user = _userRepository.findByUsername(userLoginDTO.GetUsername());
    if(user == nullptr){
        return QJsonObject(
        {
            qMakePair(QString("status"), QJsonValue("error")),
            qMakePair(QString("errors"), QJsonValue("not_found"))
        });
    }

    QJsonWebToken jwt = QJsonWebToken();
    jwt.setRandomSecret();

    jwt.setSecret("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
    // set a default payload
    jwt.appendClaim("iss", "juangburgos");
    ...
    jwt.appendClaim("aud", "everybody");
    jwt.appendClaim("sub", "hey there");


    QString jwtString = jwt.getToken();
    QStringList listJwtParts = jwtString.split(".");
    // check correct size
    if (listJwtParts.count() == 3)
    {
        QJsonWebToken token = QJsonWebToken::fromTokenAndSecret(jwtString, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");

        QString strHeader = token.getHeaderQStr();
        QString strPayload = token.getPayloadQStr();
        if (token.isValid())
        {
            qDebug() << "jwt is VALID strPayload: " << strPayload;
        }
        else
        {
            qDebug() << "jwt is INVALID";
        }
    }

    QString strValue = "123";
    QByteArray pswNsalt (strValue.toStdString().c_str()) ;
    pswNsalt.append("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX") ;
    QString hashedSaltedPsw = QCryptographicHash::hash(pswNsalt, QCryptographicHash::Sha256).toHex() ;
    qDebug() << "encrypted psw: " << hashedSaltedPsw << Qt::endl ;


    return QJsonObject(
    {
        qMakePair(QString("status"), QJsonValue("success")),
        qMakePair(QString("access_token"), QJsonValue(jwt.getToken()))
    });
